import {TreeNodeWrapper2} from "./tree-node-wrapper2";


export class TreeWrapper2<T> {

  constructor(public root: TreeNodeWrapper2<T>) {
    this.flatNodes.push(root)
  }

  public flatNodes: TreeNodeWrapper2<T>[] = []

  addFlatNode(node: TreeNodeWrapper2<T>) {
    this.flatNodes.push(node)
  }

  addFlatNodes(nodes: TreeNodeWrapper2<T>[]) {
    this.flatNodes.push(...nodes)
  }

  hierarchy(flatNodes: TreeNodeWrapper2<T>[] = this.flatNodes, parentNodeIdx = 0, treeLength = 1): number {
  ...
  hierarchy2(flatNodes: TreeNodeWrapper2<T>[] = this.flatNodes, parentNodeIdx = 0, treeLength = 1): number {
    let parentNode = flatNodes[parentNodeIdx];
    let curParentId = parentNode.getId();
    let length = this.flatNodes.length
    for (let i = treeLength; i < length; i++) {
  ...
  removeNode(node : string | TreeNodeWrapper2<T>, recursive = false) : boolean{
